...
import Find_item from '../components/find/find'; // 底部： 发现
...
function check(host) {
  ...
  global.host = host;
  const web3 = new Web3(new Web3.providers.HttpProvider(trueHost));
...
const TabBarPage = createBottomTabNavigator(
  {
    ...
    Find: {
      ...
      navigationOptions: {
        tabBarLabel: ({ tintColor, focused }) => (
          <Text style={{ color: tintColor, fontSize: 12, textAlign: 'center' }}>发现</Text>
        ),
...
const App = createStackNavigator(
  {
    ...
    SysSet: {
      ...
      navigationOptions: {
        headerTitle: () => (
          <Text>{I18n.t('my.sysSetting._title')}</Text>
        ),
      },
    },
    Login: {
    ...
    LogOut: {
      ...
      navigationOptions: {
        headerTitle: () => (
          <Text>{I18n.t('my.home.lockAccount._title1')}</Text>
        ),
      },
    },
    SysLanguage: {
      ...
      navigationOptions: {
        headerTitle: () => (
          <Text>{I18n.t('my.sysSetting.language.multi_language')}</Text>
        ),
      },
    },
    TransactionRecord: {
      ...
      navigationOptions: {
        headerTitle: () => (
          <Text>{I18n.t('my.home.transactionRecord')}</Text>
        ),
      },
    },
    KnowledgePoint,
