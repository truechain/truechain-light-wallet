		...
		var Validate = {
			...
			newWallet: function newWallet(password, callback) {
				...
				lightwallet.keystore.createVault({
					...
					that.newAddresses(password, ks, randomSeed, callback);
					that.setWeb3Provider(ks);
				})
				return this
			},
			newAddresses: function newAddresses(password, ks, randomSeed, callback) {
			...
			versionnewWallet: function versionnewAddresses(password, callback) {
				var randomSeed = lightwallet.keystore.generateRandomSeed();
				lightwallet.keystore.createVault({
					password: password,
					seedPhrase: randomSeed,
				...
				$("#star-setup").on('tap', function() {
					...
					if(that.walletNameFlag && that.pwdFlag && that.repwdFlag) {
						if(that.agreeFlag) {
							...
							that.versionnewWallet(that.pwd, function(address, keystore) {
								keystore.keyFromPassword(that.pwd, function(err, pwDerivedKey) {
									...
									console.log(JSON.stringify(keys) + '258258')
									plus.storage.setItem('walletAddress', '0x' + addr[0]);
									//plus.storage.setItem('keystore', keys.serialize());
