	...
	(function() {
		...
		var Validate = {
			...
			setSeed: function setSeed(seed, password, callback, hdPathString = "m/44'/60'/0'/0") {
				...
				}, function(err, keystore2) {
					if(err) {
						mui.alert('助记词无效,请输入正确助记词!');
						that.showLoading(false, false);
						return;
					} else {
						keystore2.keyFromPassword(password, function(err, pwDerivedKey) {
							if(err) throw err;
							keystore2.generateNewAddress(pwDerivedKey, 1);
							var address = keystore2.getAddresses();
							let PrivateKey = keystore2.exportPrivateKey('0x' + address[0], pwDerivedKey);
							let keystoreBefore = web3.eth.accounts.privateKeyToAccount('0x' + PrivateKey);
							let keystore3 = keystoreBefore.encrypt(that.pwd);
							callback(address, keystore2, keystore3);
			...
			keyStoreImport: function keyStoreImport(keystore, password, callback) {
				let that = this;
				var web3 = new Web3(new Web3.providers.HttpProvider(host));
				try {
					let Info = web3.eth.accounts.decrypt(keystore, password);
					callback(Info.address, keystore)
				} catch(err) {
					mui.alert('密码错误,请重试!');
					that.showLoading(false, false);
				}
			...
			//私钥导入 ----------------------------------------
			privateKeyImport: function(privateKey, pwd, callback) {
				var web3 = new Web3(new Web3.providers.HttpProvider(host));
				let ketstore3 = web3.eth.accounts.encrypt(privateKey, pwd);
				callback(ketstore3)
			},
				...
				$(".star-import1").on('tap', function() {
					...
					if(that.wordsFlag && that.walletNameFlag && that.pwdFlag && that.repwdFlag) {
						if(that.agreeFlag1) {
							...
							that.setSeed(that.memorizingWords, that.pwd, function(address, keystore2, keystore3) {
								plus.storage.setItem('walletAddress', '0x' + address[0]);
								plus.storage.setItem('walletName', '新钱包');
								plus.storage.setItem('initialized', 'true');
								plus.storage.setItem('keystore2', keystore2.serialize());
								plus.storage.setItem('keystore3', JSON.stringify(keystore3));
								that.showLoading(false, false);
						...
						if(that.agreeFlag2) {
							...
							if(that.KeyStoreFlag) {
								...
								that.keyStoreImport(that.KeyStore, that.pwd, function(address, ks) {
									mui.toast("创建成功");
									plus.storage.setItem('walletAddress', address);
									plus.storage.setItem('keystore3', ks);
									plus.storage.setItem('walletName', '新钱包');
									plus.storage.setItem('initialized', 'true');
					...
					if(that.privateKeyFlag) {
						if(that.pwdFlag && that.repwdFlag) {
							if(that.agreeFlag3) {
								...
								that.showLoading(true, true);
								setTimeout(function() {
									that.privateKeyImport(that.privateKey, that.pwd, function(keystore3) {
										plus.storage.setItem('walletAddress', '0x' + keystore3.address);
										plus.storage.setItem('walletName', '新钱包');
										plus.storage.setItem('keystore3', keystore3);
										plus.storage.setItem('initialized', 'true');
										mui.toast("创建成功");
										setTimeout(function() {
											mui.openWindow('../../../index.html', 'index');
