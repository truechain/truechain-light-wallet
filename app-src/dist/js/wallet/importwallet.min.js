...
(function() {
	...
	var Validate = {
		...
		newAddresses: function newAddresses(password, seed, ks, callback) {
			let that = this;
			var numAddr = 1;
			that.global_keystore.keyFromPassword(password, function(err, pwDerivedKey) {
				that.global_keystore.generateNewAddress(pwDerivedKey, numAddr);
				var addresses = that.global_keystore.getAddresses();
				callback(addresses, seed, ks)
			})
		},
		setSeed: function setSeed(seed, password, callback, hdPathString = "m/44'/60'/0'/0") {
			...
			lightwallet.keystore.createVault({
				...
				that.newAddresses(password, seed, ks, callback);
				that.setWeb3Provider(that.global_keystore);
			})
		},
				...
				if(that.wordsFlag && that.walletNameFlag && that.pwdFlag && that.repwdFlag) {
					if(that.agreeFlag1) {
						...
						that.setSeed(that.memorizingWords, that.pwd, function(address, seed, ks) {
							...
							plus.storage.setItem('ks', JSON.stringify(ks));
