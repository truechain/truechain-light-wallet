			...
			submitForm: function submitForm() {
				...
				$('.comfirmPsw').on('tap', function() {
					...
					if(that.pswFlag) {
						...
						//$('#modal').addClass('mui-active');											
						function sendEth() {
							var keystore = JSON.parse(plus.storage.getItem('keystore3'));
							let account = web3.eth.accounts.decrypt(keystore, that.pwd);
							web3.eth.accounts.wallet.add(account);
							web3.eth.sendTransaction({
								from: that.fromAddress,
								to: that.toAddress,
								value: parseFloat(that.amount) * 1.0e18,
						...
						if(currencyName == 'ETH') {
							...
							let amountWei = parseFloat(web3.utils.toWei(that.amount.toString(), 'ether'));
							mui.toast('请稍后!');
							sendTokens(that.fromAddress, that.toAddress, amountWei, that.pwd, keystore, that.trueContractAddr, mask);
							console.log('TRUE转账');
						} else if(currencyName == 'TTR') {
							let keystore = plus.storage.getItem('keystore3');
							let amountWei = parseFloat(web3.utils.toWei(that.amount.toString(), 'ether'));
							mui.toast('请稍后!');
							sendTokens(that.fromAddress, that.toAddress, amountWei, that.pwd, keystore, that.ttrContractAddr, mask);
							console.log('TTR转账');
						};
					}
				});
