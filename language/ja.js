...
export default {
	guide: {
		...
		importInstructions:
			'既存のウォレットからニーモニックをアウトポートして、「ウォレットインポート」をクリックして、アウトポートされたニーモニックを入力する。ウォレットのインポート後、速やかにニーモニックをバックアップしてください。',
		createInstructions:
			'ウォレットがない場合、「新規ウォレット」をクリックしてください。ウォレットを新規作成するとき、パスワードを覚えてください。ウォレットの新規作成後、速やかにニーモニックをバックアップしてください。'
	},
	wallet: {
		mnemonic: 'ニーモニック',
	...
	activity: {
		_title: 'アクティビティ',
		_signIn: {
			_title: 'あなたは7日間連続してボーナスパッケージを手に入れることができます。',
			todayIntegral: '今日のチェックインポイント'
		},
		signIn: 'サインイン',
		SignedIn: 'でチェック'
	},
	...
	assets: {
		...
		walletInfo: {
			...
			keystore_save_item:
				'keystoreファイルを安全、オフラインの場所に複写、貼り付けて保存していください。メールボックス、メモ帳、ネットディスク、チャットツールなどに保存しないでください。非常に危険です',
			keystore_network: 'ネットワーク伝送を使用しないでください',
			keystore_network_item:
				'ネットワークツールで keystore ファイルを伝送しないでください。万が一ハッカーに取得されると、挽回できない資産損失になりますので、オフライン方式でQRコードで伝送することをお勧めします。',
	...
	node: {
		...
		signUp_item: {
			...
			standNode_info:
				'スタンダードノードはPOWノードである。メインネットテスト時期が終了した後、ノード数量制限がなくなり、POWノード任務のみを実行する。個人キャンペーンには2千のTRUEが必要であり、かつロック位置を実行する必要があり、チームキャンペーンには3千のTRUEが必要であり、かつロック位置を実行する必要がある。'
		},
		personSignUp: {
	...
	public: {
		...
		mnemonic_ps:
			'PS: ユーザーはバックアップしたニーモニックを用いて、トゥルーチェーンウォレットをインポートすることができる。新しいパスワードで新しいKeystoreを生成させることにより、ウォレットパスワードを変更することができる。',
		keystore:
		...
		keystore_ps:
			' PS: Keystore のパスワードが唯一で変更不可能である。ウォレットパスワードを変更するには、ニーモニックまたはクリアテキストプライベートキーでウォレットを新たにインポートし、かつ新パスワードで暗号化させ、新たなKeystoreを生成させる。',
		privateKey:
			'ウォレットの資金へのコントロールは、対応のプライベートキーへの所有権及び制御権によって決められるとよく言われている。ブロックチェーン取引において、 プライベートキーが通貨支払いに必要なサインを生成させて、資金の所有権を証明するためのものである。プライベートキーは終始機密に保管すべきである。一旦第三者に漏れると、このプライベートキーに保護された資産も相手に譲ることになってしまう。 Keystoreとは違うものである。Keystoreは暗号化されたプライベートキーファイルである。パスワードの強さが十分あれば、ハッカーがKeystoreを手に入れたとしても、解読する難しさもかなりある。プライベートキーは実質的にネットワークに保存されず、ユーザーが生成してファイルまたは簡単なデータベースに保存されているものであり、ウォレットと呼ばれている。ユーザーウォレットに保存されたプライベートキーが完全に独立し、ユーザーのウォレットソフトウェアにより生成され、管理される。ブロックチェーンまたはネットワーク接続を必要としない。ユーザーのウォレットアドレスは、プライベートキーが楕円曲線暗号により公式キーを生成させ、さらに 0xから始まる 42桁のアドレスを生成させる。プライベートキーのスタイルは、64桁16小数のハッシュ文字列である。例えば、 ```56f759ece75f0ab1b783893cbe390288978d4d4ff24dd233245b4285fcc31cf6である。```',
		privateKey_ps:
			'PS: ユーザーはクリアテキストプライベートキーでトゥルーチェーンをインポートすることができ、新たなパスワードで新たな Keystoreを生成させる(古いKeystoreを削除することを忘れないで)ことにより、ウォレットパスワードを変更する。'
	},
	prompt: {
